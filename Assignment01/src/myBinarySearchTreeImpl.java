...
public class myBinarySearchTreeImpl<T1 extends Comparable<? super T1>, T2> implements myBinarySearchTree<T1, T2> {
	...
	public int my_count_at_level(int level){
		...
		if (my_is_empty() == true) 
			scenario = 1;
		else if(my_length() == 1 || level == 1)
			scenario = 2;
		else if(my_length() < level )
			scenario = 3;
		else
			scenario = 4;
		...
		//Tree is empty therefore there are 0 nodes at level
		case 1: 
			res = 0;
			break;
			
		//Tree only has 1 level, which is the root, or else the selected level is 1. This means there is only 1 node.
		case 2:
			res = 1;
			break;
			
		//Tree does not have this many levels therefore there are no nodes at this level
		case 3:
			res = 0;
			break;
			
		case 4: 
			//Checks to see if the tree is empty. If so, add nothing to res.
			//If it isnt empty, add 1 to res
			if(this.my_is_empty() == true) {
				res = res +0;
			}
			else {
			...
			//Solve the left and right trees recursively for each smaller level until no longer possible.
			//Add the totals of the left and right trees and return the total.
			int res1 = this.my_left_tree().my_count_at_level(level-1);
			int res2 = this.my_right_tree().my_count_at_level(level-1);
			res = res1 + res2;
	...
	public boolean my_is_balanced(){
		...
		//Tree is empty therefore it is balanced
		case 1: 
			res = true;
			break;
			
		//Tree only has 1 level, which is the root. This means is is balanced.
		case 2:
			res = true;
			break;
		...
		case 3:
		//Assign the left and right trees to variables
		myBinarySearchTree<T1, T2> leftTree = my_left_tree();
		myBinarySearchTree<T1, T2> rightTree = my_right_tree();
		
		//Get the length of both trees
		int leftLength = leftTree.my_length();
		int rightLength = rightTree.my_length();
		
		//If the length of the right tree is equal to the length of the left tree or only varies by 1, then it is balanced.
		//This needs to be solved recursively because the right and left sub-trees also need to be balanced.
		if(rightLength == leftLength || rightLength ==(leftLength+1) || rightLength ==(leftLength-1)) {
			if( ((leftTree.my_is_balanced())==true) && ((rightTree.my_is_balanced())==true)) {
				res = true;
			}
	...
	public int my_count_smaller_nodes(T1 key){
		...
		myBinarySearchNode<T1,T2> root = my_root();
		T1 rootKey = root.getKey();		
		int res = 0;
		...
		//Tree only has 1 level, which is the root. This means there is only 1 node.
		case 2:
		res = 0;
		break;
		
		//The key is equal to the root key. This means you can just count the nodes to the left of the root
		case 3: 
		res = this.my_left_tree().my_node_count();
		break;
			
		//The key is greater than the root key. This means you need to get the count of the nodes to the left of the root and then recursively solve the right tree.
		case 4:
		res = my_left_tree().my_node_count();
		res += my_right_tree().my_count_smaller_nodes(key);
		break;
		
		//The key is smaller than the root key. This means you need to recursively solve the left tree.
		case 5:
		res = my_left_tree().my_count_smaller_nodes(key);
		break;
